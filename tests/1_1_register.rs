...
use otto::{crdt::Crdt, settable::Settable as Register, StateTest};

fn test_register<T: StateTest>(rng: &mut impl Rng) {
...
fn fuzz_register() {
	...
	for i in 0..u64::MAX {
		...
		test_register::<Register<u8>>(rng);
	}
}

#[test]
fn fuzz_register_short() {
	let seed = rand::random();
	println!("seed: {seed}");
	let rng = &mut SmallRng::seed_from_u64(seed);
	for _ in 0..100 {
